...
/**
 * Icon Sync Script
 * Copies Lucide icons from node_modules to src/icons/lucide/
...
import { readdirSync, copyFileSync, writeFileSync, readFileSync, mkdirSync, existsSync } from 'fs';
...
const TARGET_DIR = join(ROOT, 'src', 'icons', 'lucide');
const CUSTOM_DIR = join(ROOT, 'src', 'icons', 'custom');
...
function main() {
    ...
    console.log(`${colors.dim}Syncing Lucide icons to src/icons/lucide/${colors.reset}`);
    console.log();
    ...
    console.log();
    console.log(`${colors.green}Done!${colors.reset} Icons are ready in src/icons/lucide/`);
    console.log(`${colors.dim}Add custom icons to src/icons/custom/${colors.reset}`);
}
