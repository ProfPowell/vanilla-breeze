...
| `version-switcher` | **Phases 1+2 Shipped** as `version-switcher` (`src/web-components/version-switcher/`). All three actions live: `navigate` (default), `swap` (in-place fetch + replace via View Transitions; updates `vb:version` meta), `diff` (line-level LCS diff inside `<change-set>`). `data-versioned-region` selector + `data-diff-position`. Phase 3 (provenance integration: `data-page-info-target`, banner, meta-tag fallback) is `vanilla-breeze-j6uo` — still open. Build-pipeline sibling for `vb:versions-manifest` meta is `vanilla-breeze-smb8`. Docs: [reference](/docs/elements/web-components/version-switcher/). Plan: `admin/r-n-d/version-switcher.md`. Phase 1 bead: `vanilla-breeze-jusx` (closed). Phase 2 bead: `vanilla-breeze-c42e`. | |
