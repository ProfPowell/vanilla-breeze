...
| `version-switcher` | New (planned, phased) | **Bigger than the wishlist line implied.** Two modes in one component: **release versions** (classic docs `/v1` ↔ `/v2` selector) AND **per-page edit history** (time-travel through how this URL evolved). Three switch actions: `navigate` (default), `swap` (in-place fetch + replace, View Transition), `diff` (renders via `<change-set>` tracking view). Pluggable data source: inline JSON > `data-src` URL > `<meta name="vb:versions-manifest">` fallback. Integrates with `<page-info>` (mounts inside its panel via `data-page-info-target`) and the existing `vb:version` provenance system. Phased build: Phase 1 = core + navigate (`vanilla-breeze-jusx`); Phase 2 = swap + diff (`vanilla-breeze-c42e`); Phase 3 = provenance integration (`vanilla-breeze-j6uo`); plus build-pipeline sibling for `vb:versions-manifest` meta wiring (`vanilla-breeze-smb8`). Plan: `admin/r-n-d/version-switcher.md`. |
