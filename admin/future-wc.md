...
| `comment-thread` | New (composes existing primitives) | Persistent threaded-discussion container. Author renders `<article data-comment>` children with metadata; component decorates with author header, relative timestamp, action row (Reply / Edit / Delete) + threaded indentation. Composes `<comment-box>` (reply form), `<reaction-bar>` (per-comment reactions), `<markdown-viewer>` (body rendering). **Keeps `<comment-wc>` stub** — this is additive, not a rename. Plan: `admin/r-n-d/comment-thread.md`. Bead: `vanilla-breeze-k4m5` (depends on `vanilla-breeze-tyhk`). |
...
| `comment-box` | New | Form-associated comment-form primitive composing `<markdown-editor>` + Submit/Cancel buttons. Used standalone OR as the reply-form template inside `<comment-thread>`. Presentational; emits `comment-box:submit` (cancellable). Plan: `admin/r-n-d/comment-box.md`. Bead: `vanilla-breeze-tyhk`. |
...
| `activity-feed` | New | WAI-ARIA-Feed timeline of user actions with relative timestamps. Author renders `<article data-activity data-time=...>` children. Optional date grouping (`data-group="day|week"`), optional infinite-scroll sentinel (`data-infinite` → `:load-more` event). Distinct from `comment-thread` (immutable, one-way), `recently-visited` (browser history scope), `time-index` (changelog scope). Plan: `admin/r-n-d/activity-feed.md`. Bead: `vanilla-breeze-z3yw`. |
| `poll-wc` | New | Voting + live results: per-option counts, own-vote flag, single/multi-choice, closed state. Mirrors the `reaction-bar` author-owned-state pattern (presentational; emits `poll-wc:vote`; author calls `setCount(option, count, { mine })` after server confirms). Result bars + total-vote count. WAI-ARIA radiogroup / checkboxgroup. Plan: `admin/r-n-d/poll-wc.md`. Bead: `vanilla-breeze-xjm7`. |
