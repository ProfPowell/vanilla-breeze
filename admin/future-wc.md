...
| `version-switcher` | **Phase 1 Shipped** as `version-switcher` (`src/web-components/version-switcher/`). Inline data (`data-versions` JSON or `<script type="application/json" data-versions>`), trigger + picker popover composing `<pop-over>`, `navigate` action only, mode auto-detected (releases / history). Phase 2 (swap + diff actions) is `vanilla-breeze-c42e`; Phase 3 (provenance integration via `data-page-info-target`, banner, meta-tag fallback) is `vanilla-breeze-j6uo` — both still open. Build-pipeline sibling for `vb:versions-manifest` meta is `vanilla-breeze-smb8`. Docs: [reference](/docs/elements/web-components/version-switcher/). Plan: `admin/r-n-d/version-switcher.md`. Phase 1 bead: `vanilla-breeze-jusx`. | |
