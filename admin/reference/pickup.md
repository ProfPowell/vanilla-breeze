...
1. **aynl** (P3, small) — comment-wc never upgrades on built pages: it is
   extras-only with no autoload guard. Fixing it lets the reconnect spec
   cover comment-wc too. Check whether other extras-only components share
   the gap (`tests/unit/entry-parity.test.js` is the oracle).
2. **t8ao** (P2, design) — themes are unlayered / in the tokens layer; make
   them use the `bundle-theme` layer. Load-bearing for everything after.
3. **aoku** (P2) — merge the 8 element/attribute layout CSS forks via
   `:is()`. Watch specificity: `:is()` takes the max of its arguments (bit
   the `[data-container]` override once already).
4. Then the epic's P3s: 6xxy, jhku, tfcw, dtw6, 822u, 7371, iocg, j1hi,
   p0wn, hl9c, vqw8, qoi8, vjpn.
5. Outside the epic, the P2s worth a look: kdkb (vendor `marked`), iwuq
...
- **Built pages load the autoload bundle, not main.js.** A component that
  works at vb.test in dev can be inert on the built site if it is missing
  from `src/lib/lazy-guards*.js`. That is the aynl class of bug.
