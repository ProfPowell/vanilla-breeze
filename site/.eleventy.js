...
export default function(eleventyConfig) {
  ...
  eleventyConfig.addPassthroughCopy({ [join(__dirname, 'public/favicon.svg')]: 'favicon.svg' });
  eleventyConfig.addPassthroughCopy({ [join(__dirname, 'public/favicon-32x32.png')]: 'favicon-32x32.png' });
  eleventyConfig.addPassthroughCopy({ [join(__dirname, 'public/apple-touch-icon.png')]: 'apple-touch-icon.png' });
  eleventyConfig.addPassthroughCopy({ [join(__dirname, 'public/pagefind')]: 'pagefind' });

  // ----- Watch targets -----
  // Rebuild when VB source changes
  eleventyConfig.addWatchTarget(join(ROOT, 'src'));
  eleventyConfig.addWatchTarget(join(ROOT, 'demos'));
