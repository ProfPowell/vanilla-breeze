var b=new Map,f=null,S=window.matchMedia("(prefers-reduced-motion: reduce)");function x(){f||(f=new MutationObserver(i=>{for(let e of i){for(let t of e.addedNodes)t.nodeType===Node.ELEMENT_NODE&&(_(t),t.querySelectorAll?.("*").forEach(_));for(let t of e.removedNodes)t.nodeType===Node.ELEMENT_NODE&&(k(t),t.querySelectorAll?.("*").forEach(k))}}),f.observe(document.body,{childList:!0,subtree:!0}))}function _(i){for(let[,e]of b)i.matches?.(e.selector)&&(S.matches&&e.reducedMotionFallback?e.reducedMotionFallback(i):e.init(i))}function k(i){for(let[,e]of b)i.matches?.(e.selector)&&e.destroy&&e.destroy(i)}function v(i,e){b.set(i,e),document.querySelectorAll(e.selector).forEach(t=>{S.matches&&e.reducedMotionFallback?e.reducedMotionFallback(t):e.init(t)}),document.body?x():document.addEventListener("DOMContentLoaded",x,{once:!0})}var z=new Map;function C(i,e,t={}){let a=t.priority??10,o={impl:e,bundle:t.bundle,contract:t.contract,priority:a},r=z.get(i);if(customElements.get(i)){if(!r||r.priority>=a){r&&r.priority===a&&r.impl!==e&&console.warn(`[VB Bundle] Tag <${i}> already registered by "${r.bundle}" (priority ${r.priority}). Skipping "${t.bundle}".`);return}console.warn(`[VB Bundle] Tag <${i}> defined by "${r.bundle}" cannot be replaced (customElements.define is permanent). "${t.bundle}" has higher priority but arrived late.`);return}if(r&&r.priority>=a){r.priority===a&&console.warn(`[VB Bundle] Tag <${i}> already registered by "${r.bundle}". Skipping "${t.bundle}" (first wins at equal priority).`);return}z.set(i,o),customElements.define(i,e)}var h=" ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,:!?/-",A=60,M=30,q=80;function L(){let i=document.createElement("span");return i.className="vb-flap-char",i.innerHTML=`
  ...
  `,i}function T(i,e){for(let t of i.querySelectorAll(".vb-flap-text"))t.textContent=e||"\xA0"}function P(i,e,t){return new Promise(a=>{let o=i.querySelector(".vb-flap-top .vb-flap-text"),r=i.querySelector(".vb-flap-bottom .vb-flap-text"),n=i.querySelector(".vb-flap-flip-top"),s=n.querySelector(".vb-flap-text"),l=i.querySelector(".vb-flap-flip-bottom"),c=l.querySelector(".vb-flap-text");o.textContent=e||"\xA0",r.textContent=e||"\xA0",s.textContent=e||"\xA0",c.textContent=t||"\xA0",n.classList.add("vb-flap-flipping"),l.classList.add("vb-flap-flipping"),setTimeout(()=>{o.textContent=t||"\xA0"},A/2),setTimeout(()=>{r.textContent=t||"\xA0",n.classList.remove("vb-flap-flipping"),l.classList.remove("vb-flap-flipping"),a()},A)})}async function R(i,e,t,a,o){let r=0;for(let n=0;n<t;n++){if(o._flipGeneration!==a)return;let s=n===t-1,l=h[r%h.length],c=s?e:h[(r+1)%h.length];await P(i,l,c),r++,s||await new Promise(d=>setTimeout(d,M))}}function E(i,e){let t=(i._flipGeneration||0)+1;for(i._flipGeneration=t;i.firstChild;)i.removeChild(i.firstChild);i.classList.add("vb-flap-board");let a=[];for(let o=0;o<e.length;o++){let r=L();i.appendChild(r),a.push(r)}return i._flipboardCells=a,{cells:a,generation:t}}async function B(i,e,t,a){let o=t.map((r,n)=>new Promise(s=>{setTimeout(()=>{if(i._flipGeneration!==a){s();return}let l=4+Math.floor(Math.random()*7);R(r,e[n],l,a,i).then(s)},n*q)}));await Promise.all(o),i._flipGeneration===a&&t.forEach((r,n)=>T(r,e[n]))}function $(i){let e=(i.textContent||"").trim().toUpperCase();if(!e)return;i._flipboardOriginal=e;let{cells:t,generation:a}=E(i,e);B(i,e,t,a)}function F(i){i._flipGeneration=(i._flipGeneration||0)+1,i._flipboardOriginal&&(i.textContent=i._flipboardOriginal,i.classList.remove("vb-flap-board"))}v("flipboard",{selector:"[data-flipboard]",init:$,destroy:F,reducedMotionFallback(i){let e=(i.textContent||"").trim().toUpperCase();if(!e)return;i._flipboardOriginal=e;let{cells:t}=E(i,e);t.forEach((a,o)=>T(a,e[o]))}});v("stamp-filter",{selector:"[data-stamp]",init(){if(document.getElementById("vb-stamp-noise"))return;let i=document.createElementNS("http://www.w3.org/2000/svg","svg");i.setAttribute("aria-hidden","true"),i.style.cssText="position:absolute;width:0;height:0;overflow:hidden",i.innerHTML=`
      <defs>
      ...
      <div part="bezel">
        ...
        <div part="screen" aria-hidden="true">
          ...
          <div class="vu-row" aria-hidden="true">
            ${Array.from({length:8},(a,o)=>`<div part="vu-bar" class="vu-bar" data-index="${o}">
    ...
    `}_upgradeAudio(){let t=this.shadowRoot.querySelector("slot")?.assignedElements?.().find(o=>o.tagName==="AUDIO");t?this._audio=t:this._audio=this.shadowRoot.querySelector(".native-audio");let a=this.getAttribute("src");a&&(this._audio.src=a),this.hasAttribute("autoplay")&&(this._audio.autoplay=!0),this.hasAttribute("loop")&&(this._audio.loop=!0),this._onTimeUpdate=()=>this._handleTimeUpdate(),this._onEnded=()=>this._handleEnded(),this._audio.addEventListener("timeupdate",this._onTimeUpdate),this._audio.addEventListener("ended",this._onEnded),this._audio.addEventListener("loadedmetadata",()=>this._updateDuration())}_setupAudioContext(){this._audioCtx||(this._audioCtx=new AudioContext,this._analyser=this._audioCtx.createAnalyser(),this._analyser.fftSize=256,this._analyser.smoothingTimeConstant=.8,this._source=this._audioCtx.createMediaElementSource(this._audio),this._source.connect(this._analyser),this._analyser.connect(this._audioCtx.destination))}_attachEvents(){let e=this.shadowRoot;e.querySelector(".btn-play").addEventListener("click",()=>{this._playing?this._pause():this._play()}),e.querySelector(".btn-skip").addEventListener("click",()=>{this._audio.currentTime=0,this._playing||this._play()});let t=e.querySelector('[part~="timeline"]');t.addEventListener("input",()=>{this._audio.duration&&(this._audio.currentTime=t.value/100*this._audio.duration)});let a=e.querySelector('[part~="volume"]');a.addEventListener("input",()=>{this._audio.volume=a.value})}_play(){this._setupAudioContext(),this._audioCtx.state==="suspended"&&this._audioCtx.resume(),this._audio.play(),this._playing=!0,this.setAttribute("data-state","playing"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-label","Pause"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-pressed","true"),this.dispatchEvent(new CustomEvent("vb:audioplayer:play",{bubbles:!0,composed:!0,detail:{currentTime:this._audio.currentTime,src:this._audio.src}}))}_pause(){this._audio.pause(),this._playing=!1,this.setAttribute("data-state","paused"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-label","Play"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-pressed","false"),this.dispatchEvent(new CustomEvent("vb:audioplayer:pause",{bubbles:!0,composed:!0,detail:{currentTime:this._audio.currentTime}}))}_handleTimeUpdate(){let e=this._audio.currentTime,t=this._audio.duration||0,a=t?e/t*100:0;this.shadowRoot.querySelector(".current-time").textContent=this._formatTime(e);let o=this.shadowRoot.querySelector('[part~="timeline"]');o.value=a,o.setAttribute("aria-valuenow",Math.round(a)),this.shadowRoot.querySelector(".timeline-fill").style.width=`${a}%`;let r=this.shadowRoot.querySelector('[part~="controls"]');r&&(r.dataset.miniTime=`${this._formatTime(e)} / ${this._formatTime(t)}`),this.dispatchEvent(new CustomEvent("vb:audioplayer:timeupdate",{bubbles:!0,composed:!0,detail:{currentTime:e,duration:t,progress:a}}))}_updateDuration(){let e=this._audio.duration||0;this.shadowRoot.querySelector(".duration").textContent=this._formatTime(e);let t=this.shadowRoot.querySelector('[part~="controls"]');t&&(t.dataset.miniTime=`0:00 / ${this._formatTime(e)}`)}_handleEnded(){this._playing=!1,this.setAttribute("data-state","ended"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-label","Play"),this.shadowRoot.querySelector(".btn-play").setAttribute("aria-pressed","false"),this.dispatchEvent(new CustomEvent("vb:audioplayer:ended",{bubbles:!0,composed:!0,detail:{src:this._audio.src}}))}_formatTime(e){if(!isFinite(e))return"0:00";let t=Math.floor(e/60),a=Math.floor(e%60).toString().padStart(2,"0");return`${t}:${a}`}_startVisualizer(){let e=this.shadowRoot.querySelector(".visualizer");if(!e)return;let t=()=>{this._visualizerPaused||(this._rafId=requestAnimationFrame(t),this._drawFrame(e))};this._rafId=requestAnimationFrame(t)}_drawFrame(e){let t=e.getContext("2d"),a=e.width=e.offsetWidth,o=e.height=e.offsetHeight,r=this._currentMode,n=getComputedStyle(this),s=n.getPropertyValue("--audioplayer-screen-color").trim()||"oklch(70% 0.28 145)",l=n.getPropertyValue("--audioplayer-screen-bg").trim()||"oklch(6% 0.02 260)",c=null;if(this._analyser&&this._playing){let d=this._analyser.frequencyBinCount;c=new Uint8Array(d),r==="wave"?this._analyser.getByteTimeDomainData(c):this._analyser.getByteFrequencyData(c)}t.fillStyle=l,t.globalAlpha=this._playing?.25:1,t.fillRect(0,0,a,o),t.globalAlpha=1,this._playing?r==="wave"?this._drawWave(t,a,o,s,c):r==="bars"?this._drawBars(t,a,o,s,c):r==="circle"&&this._drawCircle(t,a,o,s,c):this._drawIdle(t,a,o,s),this._updateVU(c)}_drawIdle(e,t,a,o){let r=a/2;e.beginPath(),e.moveTo(0,r),e.lineTo(t,r),e.strokeStyle=o,e.lineWidth=1,e.globalAlpha=.3,e.stroke(),e.globalAlpha=1,Math.floor(Date.now()/600)%2===0&&(e.beginPath(),e.arc(12,a/2,2,0,Math.PI*2),e.fillStyle=o,e.globalAlpha=.6,e.fill(),e.globalAlpha=1)}_drawWave(e,t,a,o,r){if(!r)return;let n=t/r.length,s=0;e.beginPath(),e.shadowColor=o,e.shadowBlur=8,e.strokeStyle=o,e.lineWidth=2,e.globalAlpha=.5;for(let l=0;l<r.length;l++){let d=r[l]/128*a/2;l===0?e.moveTo(s,d):e.lineTo(s,d),s+=n}e.stroke(),e.beginPath(),e.shadowBlur=0,e.strokeStyle=o,e.lineWidth=1.5,e.globalAlpha=1,s=0;for(let l=0;l<r.length;l++){let d=r[l]/128*a/2;l===0?e.moveTo(s,d):e.lineTo(s,d),s+=n}e.stroke()}_drawBars(e,t,a,o,r){if(!r)return;let n=Math.min(r.length,48),s=t/n-1;e.shadowColor=o,e.shadowBlur=6;for(let l=0;l<n;l++){let d=r[l]/255*a,u=l*(t/n),p=e.createLinearGradient(0,a-d,0,a);p.addColorStop(0,o),e.globalAlpha=1,p.addColorStop(1,o),e.fillStyle=p,e.fillRect(u,a-d,s,d),e.globalAlpha=.3,e.fillStyle=o,e.fillRect(u,a-d/2,s,d/2)}e.globalAlpha=1,e.shadowBlur=0}_drawCircle(e,t,a,o,r){if(!r)return;let n=t/2,s=a/2,l=Math.min(t,a)*.25,c=r.length;e.shadowColor=o,e.shadowBlur=6,e.strokeStyle=o,e.lineWidth=1.5,e.beginPath();for(let d=0;d<c;d++){let u=d/c*Math.PI*2,p=r[d]/255*(l*.8),y=l+p,g=n+y*Math.cos(u),w=s+y*Math.sin(u);d===0?e.moveTo(g,w):e.lineTo(g,w)}e.closePath(),e.stroke(),e.beginPath(),e.arc(n,s,2,0,Math.PI*2),e.fillStyle=o,e.globalAlpha=.4,e.fill(),e.globalAlpha=1,e.shadowBlur=0}_drawStatic(){let e=this.shadowRoot.querySelector(".visualizer");e&&this._drawIdle(e.getContext("2d"),e.offsetWidth,e.offsetHeight,getComputedStyle(this).getPropertyValue("--audioplayer-screen-color").trim())}_updateVU(e){let t=this.shadowRoot.querySelectorAll(".vu-bar");if(!t.length)return;if(!e||!this._playing){t.forEach(o=>{o.querySelector(".vu-fill").style.height="0%",o.removeAttribute("data-peak")});return}let a=Math.floor(e.length/t.length);t.forEach((o,r)=>{let n=e[r*a]/255,s=Math.round(n*100);o.querySelector(".vu-fill").style.height=`${s}%`,o.toggleAttribute("data-peak",s>80)})}_titleFromSrc(){let e=this.getAttribute("src");return e?e.split("/").pop().split("?")[0].replace(/\.[^.]+$/,"").replace(/[-_]/g," ").toUpperCase():"NO SIGNAL"}_updateTrackTitle(){let e=this.getAttribute("data-title")||this._titleFromSrc(),t=this.shadowRoot.querySelector('[part~="track-title"]');t&&(t.textContent=e);let a=this.shadowRoot.querySelector('[part~="controls"]');a&&(a.dataset.miniTitle=e)}};C("audio-player",m,{bundle:"retro",contract:"audio-player",priority:10});
