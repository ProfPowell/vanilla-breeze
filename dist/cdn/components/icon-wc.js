...
`;var n=new Map,a=class r extends HTMLElement{static#e=new Set;static#t=null;static#i(){r.#t||(r.#t=new MutationObserver(t=>{for(let i of t)if(i.attributeName==="data-icon-set")for(let s of r.#e)s.hasAttribute("set")||(s.render(),s.loadIcon())}),r.#t?.observe(document.documentElement,{attributes:!0,attributeFilter:["data-icon-set"]}))}static get observedAttributes(){return["name","set","size","label"]}constructor(){super(),this.attachShadow({mode:"open"})}get name(){return this.getAttribute("name")||""}get set(){return this.getAttribute("set")||document.documentElement.dataset.iconSet||"lucide"}get size(){return this.getAttribute("size")||"md"}get label(){return this.getAttribute("label")}get basePath(){return this.getAttribute("base-path")||document.documentElement.dataset.iconPath||"/src/icons"}get iconPath(){return`${this.basePath}/${this.set}/${this.name}.svg`}render(){this.shadowRoot.innerHTML=`
            <style>${l}</style>
            <slot></slot>
        `}async loadIcon(){if(!this.name){this.setError("No icon name specified");return}let t=`${this.set}/${this.name}`;if(n.has(t)){this.displayIcon(n.get(t));return}try{let i=await this.#s(this.set);n.set(t,i),this.displayIcon(i)}catch(i){let s="lucide";if(this.set!==s){let e=`${s}/${this.name}`;if(n.has(e)){this.displayIcon(n.get(e));return}try{let o=await this.#s(s);n.set(e,o),this.displayIcon(o);return}catch{}}this.setError(i.message)}}async#s(t){let i=`${this.basePath}/${t}/${this.name}.svg`,s=await fetch(i);if(!s.ok)throw new Error(`Icon not found: ${this.name}`);let e=await s.text();if(!e.includes("<svg"))throw new Error(`Invalid SVG: ${this.name}`);return e}displayIcon(t){let e=new DOMParser().parseFromString(t,"image/svg+xml").querySelector("svg");if(!e){this.setError("Invalid SVG content");return}e.removeAttribute("width"),e.removeAttribute("height"),this.label?(e.setAttribute("role","img"),e.setAttribute("aria-label",this.label)):e.setAttribute("aria-hidden","true"),this.removeAttribute("data-error");let o=this.shadowRoot?.querySelector("slot");o&&o.replaceWith(e)}setError(t){this.setAttribute("data-error","true"),console.warn(`icon-wc: ${t}`);let i=this.shadowRoot?.querySelector("slot");i&&(i.textContent="")}connectedCallback(){r.#e.add(this),r.#i(),this.render(),this.loadIcon(),this._onThemeChange=()=>{let t=this.shadowRoot?.querySelector("svg");t&&(t.style.stroke="none",requestAnimationFrame(()=>{t.style.stroke=""}))},window.addEventListener("theme-change",this._onThemeChange),this.setAttribute("data-upgraded","")}disconnectedCallback(){this.removeAttribute("data-upgraded"),r.#e.delete(this),this._onThemeChange&&window.removeEventListener("theme-change",this._onThemeChange)}attributeChangedCallback(t,i,s){if(i!==s&&this.isConnected){if(t==="name"||t==="set")this.render(),this.loadIcon();else if(t==="label"){let e=this.shadowRoot?.querySelector("svg");e&&(s?(e.setAttribute("role","img"),e.setAttribute("aria-label",s)):(e.removeAttribute("role"),e.setAttribute("aria-hidden","true")))}}}};customElements.define("icon-wc",a);export{a as IconWc};
