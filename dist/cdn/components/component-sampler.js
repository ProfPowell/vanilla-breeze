var b=window.matchMedia("(prefers-reduced-motion: reduce)");var u=new Map;function p(r,a,t={}){let o=t.priority??10,n={impl:a,bundle:t.bundle,contract:t.contract,priority:o},e=u.get(r);if(customElements.get(r)){if(!e||e.priority>=o){e&&e.priority===o&&e.impl!==a&&console.warn(`[VB Bundle] Tag <${r}> already registered by "${e.bundle}" (priority ${e.priority}). Skipping "${t.bundle}".`);return}console.warn(`[VB Bundle] Tag <${r}> defined by "${e.bundle}" cannot be replaced (customElements.define is permanent). "${t.bundle}" has higher priority but arrived late.`);return}if(e&&e.priority>=o){e.priority===o&&console.warn(`[VB Bundle] Tag <${r}> already registered by "${e.bundle}". Skipping "${t.bundle}" (first wins at equal priority).`);return}u.set(r,n),customElements.define(r,a)}var i=class extends HTMLElement{#e=[];connectedCallback(){this.hasAttribute("data-upgraded")||this.setup()!==!1&&this.setAttribute("data-upgraded","")}disconnectedCallback(){for(let a of this.#e)a();this.#e=[],this.removeAttribute("data-upgraded"),this.teardown()}listen(a,t,o,n){a.addEventListener(t,o,n),this.#e.push(()=>a.removeEventListener(t,o,n))}setup(){}teardown(){}};var m={button:()=>`
    ...
    <textarea rows="3" placeholder="Textarea sample" aria-label="Sample textarea" style="width:100%"></textarea>`},c=class extends i{static observedAttributes=["components","label","compact"];setup(){this.#e()}attributeChangedCallback(){this.isConnected&&this.#e()}#e(){let a=this.getAttribute("components")||"button,input,select,checkbox,radio,badge,progress",t=this.getAttribute("label")||"",o=this.hasAttribute("compact"),n=a.split(",").map(s=>s.trim()),e=o?"0.75rem":"1.25rem",l="";t&&(l+=`<p style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--color-text-muted,#666);margin-block-end:0.75rem;font-family:var(--font-sans,system-ui)">${t}</p>`),l+=`<section style="display:grid;grid-template-columns:repeat(auto-fit,minmax(14rem,1fr));gap:${e}">`;for(let s of n){let d=m[s];d&&(l+=`<article style="border:1px solid var(--color-border,#ddd);border-radius:var(--radius-m,0.5rem);padding:var(--size-m,1rem);background:var(--color-surface,#fff)">
        <p style="font-size:0.625rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--color-text-muted,#999);margin-block-end:var(--size-s,0.5rem);font-family:var(--font-sans,system-ui)">${s}</p>
        ${d()}
      </article>`)}l+="</section>",this.innerHTML=l}};p("component-sampler",c);
