var f=/Mac|iPhone|iPad|iPod/.test(navigator.platform??"");function c(i){let t=i.toLowerCase().split("+").map(s=>s.trim());return{key:t.pop()??"",meta:t.includes("meta")||t.includes("cmd"),ctrl:t.includes("ctrl"),shift:t.includes("shift"),alt:t.includes("alt")}}function h(i,t){if(i.key.toLowerCase()!==t.key)return!1;let e,s;return f?(e=t.meta,s=t.ctrl):(e=!1,s=t.meta||t.ctrl),!(e!==i.metaKey||s!==i.ctrlKey||t.shift!==i.shiftKey||t.alt!==i.altKey)}var b=new Set(["INPUT","TEXTAREA","SELECT"]),a=[],d=!1;function m(i){let t=b.has(i.target.tagName)||i.target.isContentEditable;for(let e of a)if(!(t&&!e.global)&&h(i,e.descriptor)){i.preventDefault(),e.callback(i);return}}function g(){d||(document.addEventListener("keydown",m),d=!0)}function u(i,t,e={}){g();let s=c(i),n={combo:i,descriptor:s,callback:t,global:e.global===!0};return a.find(o=>o.combo===i)&&console.warn(`[hotkey-bind] Shortcut "${i}" already bound. Last-connected-wins.`),a.unshift(n),function(){let l=a.indexOf(n);l!==-1&&a.splice(l,1)}}function p(){return a.map(i=>i.combo)}var r=class i extends HTMLElement{static#m=150;static#g=8;#t;#e;#a;#i;#s=[];#n=-1;#r=!1;#o=null;#l=null;#h=null;connectedCallback(){this.#k(),this.#v(),this.setAttribute("data-upgraded","")}disconnectedCallback(){this.removeAttribute("data-upgraded"),this.#h?.(),document.removeEventListener("keydown",this.#d),this.#c()}#k(){this.#t=this.querySelector(":scope > [data-trigger]"),this.#t||(this.#t=this.querySelector(":scope > button")),this.#t||(this.#t=document.createElement("button"),this.#t.setAttribute("data-trigger",""),this.#t.innerHTML=`
    ...
    `,this.appendChild(this.#e),this.#a=this.#e.querySelector(".input"),this.#i=this.#e.querySelector(".results")}#v(){this.#t.addEventListener("click",this.#y),p().includes("meta+k")||(this.#h=u("meta+k",()=>{this.#r?this.close():this.open()},{global:!0})),document.addEventListener("keydown",this.#d),this.#e.querySelector(".backdrop").addEventListener("click",()=>this.close()),this.#a.addEventListener("input",this.#w),this.#a.addEventListener("keydown",this.#E),this.#i.addEventListener("click",this.#L)}#y=t=>{t.stopPropagation(),this.open()};#d=t=>{t.key==="Escape"&&this.#r&&(t.preventDefault(),this.close())};#w=()=>{this.#c(),this.#l=setTimeout(()=>{this.#A(this.#a.value)},i.#m)};#E=t=>{switch(t.key){case"ArrowDown":t.preventDefault(),this.#f(this.#n+1);break;case"ArrowUp":t.preventDefault(),this.#f(this.#n-1);break;case"Enter":t.preventDefault(),this.#n>=0&&this.#s[this.#n]&&this.#b(this.#s[this.#n]);break}};#L=t=>{let e=t.target.closest("[data-result-index]");if(e){let s=parseInt(e.dataset.resultIndex,10);this.#s[s]&&this.#b(this.#s[s])}};async#u(){if(!this.#o)try{let t=new Function('return import("/pagefind/pagefind.js")');this.#o=await t(),await this.#o.options({excerptLength:20})}catch{console.warn("Pagefind not available. Run `npm run search:dev` to build the search index."),this.#o=null}}async#A(t){if(!t.trim()){this.#p([]);return}if(await this.#u(),!this.#o){this.#S();return}try{this.#i.innerHTML='<div class="loading">Searching...</div>';let e=await this.#o.search(t),s=await Promise.all(e.results.slice(0,i.#g).map(n=>n.data()));this.#s=s,this.#p(s)}catch(e){console.error("Search error:",e),this.#i.innerHTML='<div class="error">Search error. Please try again.</div>'}}#p(t){if(this.#n=-1,t.length===0){this.#a.value.trim()?this.#i.innerHTML='<div class="empty">No results found</div>':this.#i.innerHTML="";return}this.#i.innerHTML=t.map((e,s)=>`
      <a
        href="${e.url}"
        class="result"
        role="option"
