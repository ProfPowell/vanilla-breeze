function Ar(o){return o&&o.__esModule&&Object.prototype.hasOwnProperty.call(o,"default")?o.default:o}var Ge,gt;function Cr(){if(gt)return Ge;gt=1;function o(r){return r instanceof Map?r.clear=r.delete=r.set=function(){throw new Error("map is read-only")}:r instanceof Set&&(r.add=r.clear=r.delete=function(){throw new Error("set is read-only")}),Object.freeze(r),Object.getOwnPropertyNames(r).forEach(i=>{let d=r[i],y=typeof d;(y==="object"||y==="function")&&!Object.isFrozen(d)&&o(d)}),r}class e{constructor(i){i.data===void 0&&(i.data={}),this.data=i.data,this.isMatchIgnored=!1}ignoreMatch(){this.isMatchIgnored=!0}}function t(r){return r.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;")}function n(r,...i){let d=Object.create(null);for(let y in r)d[y]=r[y];return i.forEach(function(y){for(let B in y)d[B]=y[B]}),d}let a="</span>",s=r=>!!r.scope,h=(r,{prefix:i})=>{if(r.startsWith("language:"))return r.replace("language:","language-");if(r.includes(".")){let d=r.split(".");return[`${i}${d.shift()}`,...d.map((y,B)=>`${y}${"_".repeat(B+1)}`)].join(" ")}return`${i}${r}`};class u{constructor(i,d){this.buffer="",this.classPrefix=d.classPrefix,i.walk(this)}addText(i){this.buffer+=t(i)}openNode(i){if(!s(i))return;let d=h(i.scope,{prefix:this.classPrefix});this.span(d)}closeNode(i){s(i)&&(this.buffer+=a)}value(){return this.buffer}span(i){this.buffer+=`<span class="${i}">`}}let b=(r={})=>{let i={children:[]};return Object.assign(i,r),i};class g{constructor(){this.rootNode=b(),this.stack=[this.rootNode]}get top(){return this.stack[this.stack.length-1]}get root(){return this.rootNode}add(i){this.top.children.push(i)}openNode(i){let d=b({scope:i});this.add(d),this.stack.push(d)}closeNode(){if(this.stack.length>1)return this.stack.pop()}closeAllNodes(){for(;this.closeNode(););}toJSON(){return JSON.stringify(this.rootNode,null,4)}walk(i){return this.constructor._walk(i,this.rootNode)}static _walk(i,d){return typeof d=="string"?i.addText(d):d.children&&(i.openNode(d),d.children.forEach(y=>this._walk(i,y)),i.closeNode(d)),i}static _collapse(i){typeof i!="string"&&i.children&&(i.children.every(d=>typeof d=="string")?i.children=[i.children.join("")]:i.children.forEach(d=>{g._collapse(d)}))}}class A extends g{constructor(i){super(),this.options=i}addText(i){i!==""&&this.add(i)}startScope(i){this.openNode(i)}endScope(){this.closeNode()}__addSublanguage(i,d){let y=i.root;d&&(y.scope=`language:${d}`),this.add(y)}toHTML(){return new u(this,this.options).value()}finalize(){return this.closeAllNodes(),!0}}function k(r){return r?typeof r=="string"?r:r.source:null}function _(r){return M("(?=",r,")")}function C(r){return M("(?:",r,")*")}function T(r){return M("(?:",r,")?")}function M(...r){return r.map(i=>k(i)).join("")}function W(r){let i=r[r.length-1];return typeof i=="object"&&i.constructor===Object?(r.splice(r.length-1,1),i):{}}function P(...r){return"("+(W(r).capture?"":"?:")+r.map(i=>k(i)).join("|")+")"}function H(r){return new RegExp(r.toString()+"|").exec("").length-1}function q(r,i){let d=r&&r.exec(i);return d&&d.index===0}let Z=/\[(?:[^\\\]]|\\.)*\]|\(\??|\\([1-9][0-9]*)|\\./;function z(r,{joinWith:i}){let d=0;return r.map(y=>{d+=1;let B=d,D=k(y),p="";for(;D.length>0;){let m=Z.exec(D);if(!m){p+=D;break}p+=D.substring(0,m.index),D=D.substring(m.index+m[0].length),m[0][0]==="\\"&&m[1]?p+="\\"+String(Number(m[1])+B):(p+=m[0],m[0]==="("&&d++)}return p}).map(y=>`(${y})`).join(i)}let ie=/\b\B/,J="[a-zA-Z]\\w*",Y="[a-zA-Z_]\\w*",ee="\\b\\d+(\\.\\d+)?",te="(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)",re="\\b(0b[01]+)",K="!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~",I=(r={})=>{let i=/^#![ ]*\//;return r.binary&&(r.begin=M(i,/.*\b/,r.binary,/\b.*/)),n({scope:"meta",begin:i,end:/$/,relevance:0,"on:begin":(d,y)=>{d.index!==0&&y.ignoreMatch()}},r)},j={begin:"\\\\[\\s\\S]",relevance:0},oe={scope:"string",begin:"'",end:"'",illegal:"\\n",contains:[j]},_e={scope:"string",begin:'"',end:'"',illegal:"\\n",contains:[j]},L={begin:/\b(a|an|the|are|I'm|isn't|don't|doesn't|won't|but|just|should|pretty|simply|enough|gonna|going|wtf|so|such|will|you|your|they|like|more)\b/},G=function(r,i,d={}){let y=n({scope:"comment",begin:r,end:i,contains:[]},d);y.contains.push({scope:"doctag",begin:"[ ]*(?=(TODO|FIXME|NOTE|BUG|OPTIMIZE|HACK|XXX):)",end:/(TODO|FIXME|NOTE|BUG|OPTIMIZE|HACK|XXX):/,excludeBegin:!0,relevance:0});let B=P("I","a","is","so","us","to","at","if","in","it","on",/[A-Za-z]+['](d|ve|re|ll|t|s|n)/,/[A-Za-z]+[-][a-z]+/,/[A-Za-z][a-z]{2,}/);return y.contains.push({begin:M(/[ ]+/,"(",B,/[.]?[:]?([.][ ]|[ ])/,"){3}")}),y},ne=G("//","$"),de=G("/\\*","\\*/"),he=G("#","$"),me={scope:"number",begin:ee,relevance:0},It={scope:"number",begin:te,relevance:0},jt={scope:"number",begin:re,relevance:0},Bt={scope:"regexp",begin:/\/(?=[^/\n]*\/)/,end:/\/[gimuy]*/,contains:[j,{begin:/\[/,end:/\]/,relevance:0,contains:[j]}]},Dt={scope:"title",begin:J,relevance:0},Pt={scope:"title",begin:Y,relevance:0},Ht={begin:"\\.\\s*"+Y,relevance:0};var Ce=Object.freeze({__proto__:null,APOS_STRING_MODE:oe,BACKSLASH_ESCAPE:j,BINARY_NUMBER_MODE:jt,BINARY_NUMBER_RE:re,COMMENT:G,C_BLOCK_COMMENT_MODE:de,C_LINE_COMMENT_MODE:ne,C_NUMBER_MODE:It,C_NUMBER_RE:te,END_SAME_AS_BEGIN:function(r){return Object.assign(r,{"on:begin":(i,d)=>{d.data._beginMatch=i[1]},"on:end":(i,d)=>{d.data._beginMatch!==i[1]&&d.ignoreMatch()}})},HASH_COMMENT_MODE:he,IDENT_RE:J,MATCH_NOTHING_RE:ie,METHOD_GUARD:Ht,NUMBER_MODE:me,NUMBER_RE:ee,PHRASAL_WORDS_MODE:L,QUOTE_STRING_MODE:_e,REGEXP_MODE:Bt,RE_STARTERS_RE:K,SHEBANG:I,TITLE_MODE:Dt,UNDERSCORE_IDENT_RE:Y,UNDERSCORE_TITLE_MODE:Pt});function Ut(r,i){r.input[r.index-1]==="."&&i.ignoreMatch()}function Ft(r,i){r.className!==void 0&&(r.scope=r.className,delete r.className)}function qt(r,i){i&&r.beginKeywords&&(r.begin="\\b("+r.beginKeywords.split(" ").join("|")+")(?!\\.)(?=\\b|\\s)",r.__beforeBegin=Ut,r.keywords=r.keywords||r.beginKeywords,delete r.beginKeywords,r.relevance===void 0&&(r.relevance=0))}function Zt(r,i){Array.isArray(r.illegal)&&(r.illegal=P(...r.illegal))}function Kt(r,i){if(r.match){if(r.begin||r.end)throw new Error("begin & end are not supported with match");r.begin=r.match,delete r.match}}function Gt(r,i){r.relevance===void 0&&(r.relevance=1)}let Vt=(r,i)=>{if(!r.beforeMatch)return;if(r.starts)throw new Error("beforeMatch cannot be used with starts");let d=Object.assign({},r);Object.keys(r).forEach(y=>{delete r[y]}),r.keywords=d.keywords,r.begin=M(d.beforeMatch,_(d.begin)),r.starts={relevance:0,contains:[Object.assign(d,{endsParent:!0})]},r.relevance=0,delete d.beforeMatch},Wt=["of","and","for","in","not","or","if","then","parent","list","value"],Xt="keyword";function Qe(r,i,d=Xt){let y=Object.create(null);return typeof r=="string"?B(d,r.split(" ")):Array.isArray(r)?B(d,r):Object.keys(r).forEach(function(D){Object.assign(y,Qe(r[D],i,D))}),y;function B(D,p){i&&(p=p.map(m=>m.toLowerCase())),p.forEach(function(m){let w=m.split("|");y[w[0]]=[D,Jt(w[0],w[1])]})}}function Jt(r,i){return i?Number(i):Qt(r)?0:1}function Qt(r){return Wt.includes(r.toLowerCase())}let Ye={},pe=r=>{console.error(r)},et=(r,...i)=>{console.log(`WARN: ${r}`,...i)},xe=(r,i)=>{Ye[`${r}/${i}`]||(console.log(`Deprecated as of ${r}. ${i}`),Ye[`${r}/${i}`]=!0)},Me=new Error;function tt(r,i,{key:d}){let y=0,B=r[d],D={},p={};for(let m=1;m<=i.length;m++)p[m+y]=B[m],D[m+y]=!0,y+=H(i[m-1]);r[d]=p,r[d]._emit=D,r[d]._multi=!0}function Yt(r){if(Array.isArray(r.begin)){if(r.skip||r.excludeBegin||r.returnBegin)throw pe("skip, excludeBegin, returnBegin not compatible with beginScope: {}"),Me;if(typeof r.beginScope!="object"||r.beginScope===null)throw pe("beginScope must be object"),Me;tt(r,r.begin,{key:"beginScope"}),r.begin=z(r.begin,{joinWith:""})}}function er(r){if(Array.isArray(r.end)){if(r.skip||r.excludeEnd||r.returnEnd)throw pe("skip, excludeEnd, returnEnd not compatible with endScope: {}"),Me;if(typeof r.endScope!="object"||r.endScope===null)throw pe("endScope must be object"),Me;tt(r,r.end,{key:"endScope"}),r.end=z(r.end,{joinWith:""})}}function tr(r){r.scope&&typeof r.scope=="object"&&r.scope!==null&&(r.beginScope=r.scope,delete r.scope)}function rr(r){tr(r),typeof r.beginScope=="string"&&(r.beginScope={_wrap:r.beginScope}),typeof r.endScope=="string"&&(r.endScope={_wrap:r.endScope}),Yt(r),er(r)}function or(r){function i(p,m){return new RegExp(k(p),"m"+(r.case_insensitive?"i":"")+(r.unicodeRegex?"u":"")+(m?"g":""))}class d{constructor(){this.matchIndexes={},this.regexes=[],this.matchAt=1,this.position=0}addRule(m,w){w.position=this.position++,this.matchIndexes[this.matchAt]=w,this.regexes.push([w,m]),this.matchAt+=H(m)+1}compile(){this.regexes.length===0&&(this.exec=()=>null);let m=this.regexes.map(w=>w[1]);this.matcherRe=i(z(m,{joinWith:"|"}),!0),this.lastIndex=0}exec(m){this.matcherRe.lastIndex=this.lastIndex;let w=this.matcherRe.exec(m);if(!w)return null;let V=w.findIndex((ke,He)=>He>0&&ke!==void 0),U=this.matchIndexes[V];return w.splice(0,V),Object.assign(w,U)}}class y{constructor(){this.rules=[],this.multiRegexes=[],this.count=0,this.lastIndex=0,this.regexIndex=0}getMatcher(m){if(this.multiRegexes[m])return this.multiRegexes[m];let w=new d;return this.rules.slice(m).forEach(([V,U])=>w.addRule(V,U)),w.compile(),this.multiRegexes[m]=w,w}resumingScanAtSamePosition(){return this.regexIndex!==0}considerAll(){this.regexIndex=0}addRule(m,w){this.rules.push([m,w]),w.type==="begin"&&this.count++}exec(m){let w=this.getMatcher(this.regexIndex);w.lastIndex=this.lastIndex;let V=w.exec(m);if(this.resumingScanAtSamePosition()&&!(V&&V.index===this.lastIndex)){let U=this.getMatcher(0);U.lastIndex=this.lastIndex+1,V=U.exec(m)}return V&&(this.regexIndex+=V.position+1,this.regexIndex===this.count&&this.considerAll()),V}}function B(p){let m=new y;return p.contains.forEach(w=>m.addRule(w.begin,{rule:w,type:"begin"})),p.terminatorEnd&&m.addRule(p.terminatorEnd,{type:"end"}),p.illegal&&m.addRule(p.illegal,{type:"illegal"}),m}function D(p,m){let w=p;if(p.isCompiled)return w;[Ft,Kt,rr,Vt].forEach(U=>U(p,m)),r.compilerExtensions.forEach(U=>U(p,m)),p.__beforeBegin=null,[qt,Zt,Gt].forEach(U=>U(p,m)),p.isCompiled=!0;let V=null;return typeof p.keywords=="object"&&p.keywords.$pattern&&(p.keywords=Object.assign({},p.keywords),V=p.keywords.$pattern,delete p.keywords.$pattern),V=V||/\w+/,p.keywords&&(p.keywords=Qe(p.keywords,r.case_insensitive)),w.keywordPatternRe=i(V,!0),m&&(p.begin||(p.begin=/\B|\b/),w.beginRe=i(w.begin),!p.end&&!p.endsWithParent&&(p.end=/\B|\b/),p.end&&(w.endRe=i(w.end)),w.terminatorEnd=k(w.end)||"",p.endsWithParent&&m.terminatorEnd&&(w.terminatorEnd+=(p.end?"|":"")+m.terminatorEnd)),p.illegal&&(w.illegalRe=i(p.illegal)),p.contains||(p.contains=[]),p.contains=[].concat(...p.contains.map(function(U){return nr(U==="self"?p:U)})),p.contains.forEach(function(U){D(U,w)}),p.starts&&D(p.starts,m),w.matcher=B(w),w}if(r.compilerExtensions||(r.compilerExtensions=[]),r.contains&&r.contains.includes("self"))throw new Error("ERR: contains `self` is not supported at the top-level of a language.  See documentation.");return r.classNameAliases=n(r.classNameAliases||{}),D(r)}function rt(r){return r?r.endsWithParent||rt(r.starts):!1}function nr(r){return r.variants&&!r.cachedVariants&&(r.cachedVariants=r.variants.map(function(i){return n(r,{variants:null},i)})),r.cachedVariants?r.cachedVariants:rt(r)?n(r,{starts:r.starts?n(r.starts):null}):Object.isFrozen(r)?n(r):r}var ar="11.11.1";class ir extends Error{constructor(i,d){super(i),this.name="HTMLInjectionError",this.html=d}}let Pe=t,ot=n,nt=Symbol("nomatch"),sr=7,at=function(r){let i=Object.create(null),d=Object.create(null),y=[],B=!0,D="Could not find the language '{}', did you forget to load/include a language module?",p={disableAutodetect:!0,name:"Plain text",contains:[]},m={ignoreUnescapedHTML:!1,throwUnescapedHTML:!1,noHighlightRe:/^(no-?highlight)$/i,languageDetectRe:/\blang(?:uage)?-([\w-]+)\b/i,classPrefix:"hljs-",cssSelector:"pre code",languages:null,__emitter:A};function w(c){return m.noHighlightRe.test(c)}function V(c){let v=c.className+" ";v+=c.parentNode?c.parentNode.className:"";let S=m.languageDetectRe.exec(v);if(S){let O=be(S[1]);return O||(et(D.replace("{}",S[1])),et("Falling back to no-highlight mode for this block.",c)),O?S[1]:"no-highlight"}return v.split(/\s+/).find(O=>w(O)||be(O))}function U(c,v,S){let O="",F="";typeof v=="object"?(O=c,S=v.ignoreIllegals,F=v.language):(xe("10.7.0","highlight(lang, code, ...args) has been deprecated."),xe("10.7.0",`Please use highlight(code, options) instead.
...
`).map(u=>`<span class="code-line">${u||" "}</span>`).join(""),a=this.showLines?`<div class="line-numbers" aria-hidden="true">${t.map((u,b)=>`<span>${b+1}</span>`).join("")}</div>`:"",s=this.filename?`<span class="label">${this.escapeHtml(this.language.toUpperCase())}</span><span class="filename">${this.escapeHtml(this.filename)}</span>`:`<span class="label">${this.escapeHtml(this.label)}</span>`;this.shadowRoot.innerHTML=`
      <style>${this.getStyles()}</style>
      <div class="header">
        <div class="label-container" id="code-label">
          ${s}
    ...
    `;let h=this.shadowRoot.querySelector(".copy-button");h&&h.addEventListener("click",()=>this.copyCode())}render(){let e=(this._codeContent||this.textContent).trim(),t=e.split(`
...
`),u=h.map((Y,ee)=>{let te=ee+1,re=n.has(te),K=["code-line"];if(re&&K.push("highlighted"),a){let I=t[ee]||"";I.startsWith("+")&&!I.startsWith("+++")?K.push("diff-add"):I.startsWith("-")&&!I.startsWith("---")&&K.push("diff-remove")}return`<span class="${K.join(" ")}">${Y||" "}</span>`}).join(""),b=this.showLines?`<div class="line-numbers" aria-hidden="true">${h.map((Y,ee)=>{let te=ee+1,re=n.has(te),K=[];if(re&&K.push("highlighted"),a){let I=t[ee]||"";I.startsWith("+")&&!I.startsWith("+++")?K.push("diff-add"):I.startsWith("-")&&!I.startsWith("---")&&K.push("diff-remove")}return`<span class="${K.join(" ")}">${te}</span>`}).join("")}</div>`:"",g=this.filename?`<span class="label">${this.escapeHtml(this.language.toUpperCase())}</span><span class="filename">${this.escapeHtml(this.filename)}</span>`:`<span class="label">${this.escapeHtml(this.label)}</span>`,A=this.hasAttribute("collapsed")||this.hasAttribute("max-lines"),k=h.length,_=this.maxLines,C=A&&k>_,T=this.collapsed,M=T?`calc(${_} * 1.6em + 2rem)`:"none",W=this.maxHeight?`--cb-max-height: ${this.maxHeight};`:"",P=T?`max-height: ${M};`:"";this.shadowRoot.innerHTML=`
      <style>${this.getStyles()}</style>
      <div class="header">
        <div class="label-container" id="code-label">
          ${g}
    ...
    `}render(){let e=this.codeBlocks;if(e.length===0)return;e.forEach((a,s)=>{a.setAttribute("theme",this.theme),s===this._activeIndex?a.classList.add("active"):a.classList.remove("active")});let t=e.map((a,s)=>{let h=a.getAttribute("filename"),u=a.getAttribute("label"),b=a.getAttribute("language")||"plaintext",g=h||u||b.toUpperCase(),A=s===this._activeIndex;return`
        <button
          class="tab"
          role="tab"
          aria-selected="${A}"
...
`),a=e.body.cloneNode(!0);return a.querySelectorAll("script, style").forEach(s=>s.remove()),{html:a.innerHTML.trim(),css:t.trim(),js:n.trim()}}openInCodePen(){let e=this.parseHTMLForCodePen();if(!e)return;let t={title:this.browserTitle||"CSS Demo",description:`Demo from ${this.url}`,html:e.html,css:e.css,js:e.js,editors:"110"},n=document.createElement("form");n.action="https://codepen.io/pen/define",n.method="POST",n.target="_blank";let a=document.createElement("input");a.type="hidden",a.name="data",a.value=JSON.stringify(t),n.appendChild(a),document.body.appendChild(n),n.submit(),document.body.removeChild(n),this.toggleShareMenu()}_handleClose(){if(this.isMaximized){this.toggleMaximize();return}this.toggleMinimize()}_handleKeydown(e){e.key==="Escape"&&(this.showShareMenu?this.toggleShareMenu():this.isMaximized&&this.toggleMaximize())}_createOverlay(){if(!this.overlay){if(this.overlay=document.createElement("div"),this.overlay.style.cssText=`
    ...
    `}_deviceCSS(e){let t=Rt[this.deviceColor]||Rt.midnight,n=this.getAttribute("orientation")==="landscape",a=this._getEffectiveDimensions(e),[s,h,u,b]=this._getEffectiveSafeInsets(e),g=to[e.notch]||24,A=e.homeIndicator&&!e.homeButton?ro:0,k=e.homeButton?Tt:0,_=e.notch!=="none";return`
        :host([device]) {
    ...
    `}_deviceChrome(e){let t=this.getAttribute("orientation")==="landscape",n=e.notch!=="none",a=n?e.notch:"",s=e.homeButton?"home-button":"",h=["silver","gold","white"].includes(this.deviceColor)?"light-bezel":"",u=["device-frame",a,s,h,t?"landscape":""].filter(Boolean).join(" "),b=`
    ...
    `:""}_setupDeviceScaling(){this._resizeObserver||(this._resizeObserver=new ResizeObserver(()=>this._updateDeviceScale()),this._resizeObserver.observe(this))}_updateDeviceScale(){let e=this._getDevicePreset();if(!e)return;let t=this.shadowRoot.querySelector(".device-wrapper"),n=this.shadowRoot.querySelector(".device-frame");if(!t||!n)return;let a=this.clientWidth;if(a===0)return;let s=this._getEffectiveDimensions(e),h=s.width+e.bezel*2,u=Math.max(.5,Math.min(1,a/h));this._currentScale=u,n.style.transform=`scale(${u})`;let b=e.homeButton?Tt:0,g=s.height+e.bezel*2+b;t.style.height=`${g*u}px`}_teardownDeviceScaling(){this._resizeObserver&&(this._resizeObserver.disconnect(),this._resizeObserver=null),this._currentScale=1}_escapeHtml(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}};customElements.get("browser-window")||customElements.define("browser-window",Xe);
