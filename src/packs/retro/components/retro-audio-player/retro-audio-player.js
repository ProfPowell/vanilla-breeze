...
class RetroAudioPlayer extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['src', 'data-visualizer', 'autoplay', 'loop', 'data-title', 'data-size']
  }
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div part="bezel">
        ...
        <div part="controls" class="controls" role="group" aria-label="Audio controls"
             data-mini-title="${title}" data-mini-time="0:00 / 0:00">
  ...
  _handleTimeUpdate() {
    ...
    // Update mini-mode time display
    const controls = this.shadowRoot.querySelector('[part~="controls"]')
    if (controls) {
      controls.dataset.miniTime = `${this._formatTime(t)} / ${this._formatTime(d)}`
    }
  ...
  _updateDuration() {
    ...
    const controls = this.shadowRoot.querySelector('[part~="controls"]')
    if (controls) {
      controls.dataset.miniTime = `0:00 / ${this._formatTime(d)}`
    }
  }
  ...
  _drawBars(ctx, W, H, color, data) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Dim the bottom half for depth
      ctx.globalAlpha = 0.3
      ctx.fillStyle = color
      ctx.fillRect(x, H - barH / 2, barW, barH / 2)
    }
    ctx.globalAlpha = 1
    ctx.shadowBlur = 0
  }
  ...
  _updateTrackTitle() {
    ...
    const controls = this.shadowRoot.querySelector('[part~="controls"]')
    if (controls) controls.dataset.miniTitle = title
  }
}
