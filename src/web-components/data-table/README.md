...
The `data-table` component progressively enhances HTML tables with interactive features while maintaining full functionality without JavaScript. All enhancements are opt-in via data attributes.
...
These attributes go on the `<table>` element inside `data-table`.
