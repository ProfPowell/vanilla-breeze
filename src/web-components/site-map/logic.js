/**
 * site-map: dual-mode sitemap component
 *
 * Two behaviors selected by the `mode` attribute:
 *
 *   mode="nav-enhance" (default) — lightweight enhancer of a static HTML
 *     sitemap nav: marks the current page, auto-expands its ancestor
 *     <details>, and adds expand/collapse-all controls. Optionally loads a
 *     simple nav tree from a `src` JSON ({ children: [...] }).
 *
 *   mode="ia-tree" — information-architecture visualization: enhances a
 *     nested <nav><ul><li> into an interactive IA tree with page-type
 *     badges, status indicators, expand/collapse, keyboard navigation, and
 *     an optional org-chart visual mode (data-view="visual"). Loads from a
 *     `src` JSON ({ title, pages: [...] }) and exposes a `.pages` property.
 *
 * Without JS, the nested list renders normally as a readable hierarchy.
 *
 * @attr {string}  mode    - "nav-enhance" (default) | "ia-tree"
 * @attr {string}  current - [nav-enhance] Pathname of the current page
 * @attr {string}  title   - [ia-tree] Site map heading
 * @attr {string}  src     - URL to JSON data (shape depends on mode)
 * @attr {boolean} collapsed - [ia-tree] Start all nodes collapsed
 * @attr {boolean} compact   - [ia-tree] Reduced spacing variant
 *
 * Children (ia-tree) use:
 ...
 *
 * @fires site-map:ready - [ia-tree] After setup with { nodeCount, depth }
 * @fires site-map:select - [ia-tree] Node clicked with { href, pageType, template }
 *
 * @example
 * <!-- nav-enhance (default) -->
 * <site-map current="/docs/components/button">
 *   <nav aria-label="Full site map"><!-- pre-rendered nav --></nav>
 * </site-map>
 *
 * @example
 * <!-- ia-tree -->
 * <site-map mode="ia-tree" title="Site Architecture">
 *   <nav>
 *     <ul>
...
class SiteMap extends VBElement {
  ...
  setup() {
    // Dual-mode dispatch — nav-enhance (default) stays lightweight.
    if ((this.getAttribute('mode') || 'nav-enhance') !== 'ia-tree') {
      this.#setupNavEnhance();
      return;
    }

    SiteMap.#instanceCount++;
    ...
    if (this.dataset.view === 'visual') {
      ...
      this.dispatchEvent(new CustomEvent('site-map:ready', {
        bubbles: true,
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if ((this.getAttribute('mode') || 'nav-enhance') !== 'ia-tree') return;
  ...
  #handleClick(e) {
    ...
    if (anchor) {
      this.dispatchEvent(new CustomEvent('site-map:select', {
        bubbles: true,
        detail: {
  ...
  #layoutTree(nodes, depth = 0) {
    const W = SiteMap.#NODE_W;
    const H = SiteMap.#NODE_H;
    const HG = SiteMap.#H_GAP;
    const VG = SiteMap.#V_GAP;
  ...
  #renderSvg(opts = {}) {
    ...
    if (priorContainer?.isConnected
      ...
      const name = `sm-vt-${++SiteMap.#vtCounter}`;
  ...
  #buildToolbar() {
    ...
    // Zoom in
    const zoomIn = this.#toolbarBtn(icon(SiteMap.#ICONS.zoomIn), 'Zoom in', () => {
    ...
    // Zoom out
    const zoomOut = this.#toolbarBtn(icon(SiteMap.#ICONS.zoomOut), 'Zoom out', () => {
    ...
    // Reset zoom
    const reset = this.#toolbarBtn(icon(SiteMap.#ICONS.reset), 'Reset zoom', () => {
      this.#zoom = 1;
      this.#renderSvg();
    });
    ...
    const flip = this.#toolbarBtn(icon(SiteMap.#ICONS.flip), flipLabel, () => {
    ...
    detailLabel.textContent = SiteMap.#DETAIL_LABELS[detail];
    detailWrap.appendChild(detailLabel);

    slider.addEventListener('input', () => {
      this.dataset.detail = slider.value;
      detailLabel.textContent = SiteMap.#DETAIL_LABELS[parseInt(slider.value, 10)];
      this.#renderSvg();
    });
  ...
  #calcBounds(nodes) {
    let maxX = 0, maxY = 0;
    for (const node of nodes) {
      if (node.x + SiteMap.#NODE_W > maxX) maxX = node.x + SiteMap.#NODE_W;
      if (node.y + SiteMap.#NODE_H > maxY) maxY = node.y + SiteMap.#NODE_H;
      if (node.children.length > 0 && !node.collapsed) {
  ...
  #renderNodes(svg, nodes, parentId = '') {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      const colors = wireframe
        ...
        : (SiteMap.#TYPE_COLORS[node.pageType] || SiteMap.#TYPE_COLORS.page);
      ...
      // Detail >= 3: Status dot
      if (detail >= 3 && node.status && SiteMap.#STATUS_COLORS[node.status]) {
        const dot = document.createElementNS(NS, 'circle');
        dot.setAttribute('cx', String(node.x + W / 2));
        dot.setAttribute('cy', String(node.y + H - 8));
        dot.setAttribute('r', '4');
        dot.setAttribute('fill', wireframe ? '#94a3b8' : SiteMap.#STATUS_COLORS[node.status]);
        g.appendChild(dot);
      }
  ...
  async _loadSrc(url) {
    ...
    try {
      ...
      console.warn(`[site-map] Failed to load src="${url}":`, err);
    }
  }
   ...
   *
   * Emits site-map:pages-changed { pages, source: 'property' }.
   */
  set pages(value) {
    this._setPages(Array.isArray(value) ? value : []);
    this.dispatchEvent(new CustomEvent('site-map:pages-changed', {
  ...
  // ── nav-enhance mode (lightweight static-sitemap enhancer) ──────
  ...
  #teardownNavEnhance() {
    this.querySelector(':scope > [data-sitemap-controls]')?.remove();
  }
