/**
 * foot-notes: Progressive enhancement for footnotes
 *
 * Two elements work together:
 * - <foot-note>: Inline reference that degrades to parenthetical text
 * - <foot-notes>: Container that collects and displays footnotes
 *
 * Usage:
...
class FootNotes extends HTMLElement {
  ...
  #collectRefs() {
    ...
    // 1. Haven't been enhanced by another foot-notes instance
  ...
  #render() {
    ...
    this.#refs.forEach((ref, index) => {
      const num = index + 1;
      // Scope IDs by instance to avoid conflicts with multiple foot-notes
...
class FootNote extends HTMLElement {
  // No-op class - enhanced by FootNotes
