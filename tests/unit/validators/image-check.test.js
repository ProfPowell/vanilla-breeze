...
function runImageCheck(directory) {
  try {
    const output = execSync(`node scripts/quality/image-check.js "${directory}"`, {
      cwd: projectRoot,
      encoding: 'utf-8',
  ...
  describe('File Size Validation', () => {
    it('should have 200KB size threshold configured', async () => {
      ...
      const scriptContent = await readFile(
        join(projectRoot, 'scripts/quality/image-check.js'),
        'utf-8'
      );
...
describe('Image Optimization Script', () => {
    ...
    it('should support --dry-run flag', async () => {
      ...
      try {
        const output = execSync(
          `node scripts/quality/optimize-images.js "${testDir}" --dry-run`,
          {
            cwd: projectRoot,
            encoding: 'utf-8',
          }
  ...
  describe('Output Formats', () => {
    it('should be configured to generate WebP and AVIF', async () => {
      const scriptContent = await readFile(
        join(projectRoot, 'scripts/quality/optimize-images.js'),
        'utf-8'
      );
